...
[Once it's enabled](https://redis.io/docs/latest/operate/rc/api/get-started/enable-the-api.md), you can use the REST API to create, update, and delete subscriptions, databases, and other entities.
...
You can query the `taskId` to track the state of a specific task using [`GET /v1/tasks/{taskId}`](https://redis.io/docs/latest/operate/rc/api/api-reference#tag/Tasks/operation/getTaskById).

You can also query the state of all active tasks or recently completed tasks in your account using [`GET /tasks`](https://redis.io/docs/latest/operate/rc/api/api-reference#tag/Tasks/operation/getAllTasks).
...
For example, when you provision a new subscription, use [`GET /v1/subscriptions/{subscriptionId}`](https://redis.io/docs/latest/operate/rc/api/api-reference#tag/Subscriptions-Pro/operation/getSubscriptionById), where the `{subscriptionId}` is the resource ID that you receive when the task is in the `processing-completed` state.
